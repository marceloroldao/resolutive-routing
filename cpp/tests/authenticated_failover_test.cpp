...
int main() {
    ...
    assert(first.selected_node_id && *first.selected_node_id == "node-b");

    const AuthenticatedFailureNoticeView notice{
        .request_id = "job-42",
    ...
    const auto rerouted = reroute_after_failure(router, request, {node_b, node_c}, first, failure);

    assert(rerouted.previous_node_id && *rerouted.previous_node_id == "node-b");
    assert(rerouted.selected_node_id && *rerouted.selected_node_id == "node-c");
    assert(rerouted.excluded_nodes.size() == 1);
    assert(rerouted.excluded_nodes.front() == "node-b");
    return 0;
}
